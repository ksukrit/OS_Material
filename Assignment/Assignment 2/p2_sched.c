...
int main(int argc, char const *argv[]) {
  ...
  shared_memory = shmat(shmget(ftok("p1_sched.c", 51), sizeof(proc_data), 0666),
                        NULL, 0);  // creating shared memory
  count = atoi(argv[2]);
  FILE *data_file = fopen("data_p2.txt", "wc");
  if ((shmid = shmget(ftok("p2_sched.c", 51), count * sizeof(int), 0666)) ==
      -1) {
