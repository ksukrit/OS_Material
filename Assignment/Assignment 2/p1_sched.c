...
int main(int argc, char const *argv[]) {
  int shm_time_logging =
      shmget(ftok("main_sched.c", 51), sizeof(time_logging), 0666);
  ...
  // shared_memory = (proc_data *) shmat(shmget(ftok("main_sched.c", 0x45),
  // sizeof(proc_data), 0666), NULL, 0); // creating shared memory
  ...
  if ((shmid = shmget(ftok("p2_sched.c", 51), (count + 1) * sizeof(int),
                      0666)) == -1) {
  ...
  if ((shmid_flags = shmget(ftok("p2_sched.c", 52),
                            (THREAD_COUNT) * sizeof(int), 0666)) == -1) {
