...
// function to save the output of the grid, used on a server if ssh connection
// is broken.
...
void *solve_thread(void *arg) {
  ...
  if (check(board, r, c, v, s) == 1) {
    board[r][c] = v;
    int ut =
        tc_glob <= 16 ? 1 : 0;  // limit threads to 8 change to 16 on server
    if (thread_spawner(board, s, ut, g->t_id) == 1 &&
        emptyCount(board, s) == 0 && !found) {
      g->solved = 1;
      found = 1;
...
int thread_spawner(int board[36][36], int s, int useThreads, int call_id) {
  ...
  if (reduceSpace.n != 0 && r != -1 && c != -1 && useThreads == 1) {
    for (int i = 0; i < reduceSpace.n; i++) {
      ...
      game_board[i].col = c;
      tc_glob++;
      if (pthread_create(&threads[i], NULL, solve_thread,
                         (void *)&game_board[i])) {
...
int main(int argc, char *argv[]) {
  ...
  assert(size == 1 || size == 4 || size == 9 || size == 16 || size == 25 ||
         size == 36);
  ...
  if (found == 1 && board_solve_id != -1) {
    ...
    // \n",emptyCount(board_glob[board_solve_id],size)); printf("Board solve id
    // is %d \n",board_solve_id);
